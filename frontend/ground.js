...
export default class Ground {
  constructor(scene, cellSize, cellCount, wallThickness) {
    ...
    this._handleListeningForPosition = this._handleListeningForPosition.bind(
      this
    );
  }
  _createGroundMesh() {
  ...
  startListeningForPosition(onDoneCallback) {
    document
      .getElementById("render-canvas")
      .addEventListener("mousemove", this._handleListeningForPosition);
    document.getElementById(
      "render-canvas"
    ).onclick = this._handleConfirmPosition(onDoneCallback);
  }
  ...
  _getCellIndices(globalCoordinates) {
    const xIndex = Math.floor(
      (globalCoordinates.x + this.cellCount * this.cellSize / 2) / this.cellSize
    );

    let zIndex = Math.floor(
      (globalCoordinates.z + this.wallThickness / 2) / this.cellSize * -1
    );
  ...
  _stopListeningForPosition() {
    document
      .getElementById("render-canvas")
      .removeEventListener("mousemove", this._handleListeningForPosition);
